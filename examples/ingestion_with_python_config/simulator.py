...
class Simulator:
    def __init__(self, ingestion_service: IngestionService, environment_temperature=25.0, seed=1, faults=False):
        ...
        self.battery_loss_rate = .4 
        self.battery_charge_rate = 1
        ...
        self.camera_states = {
            "camera_1": {"current": 0.0, "temperature": self.environment_temperature, "led_pwm": 0},
            "camera_2": {"current": 0.0, "temperature": self.environment_temperature, "led_pwm": 0}
        }
        self.battery_charge_level = 99.3
        if self.faults:
    ...
    def update_camera(self, cam, on_state, fault=False):
        if on_state:
            cam["current"] = self.apply_variation(60, 15)  # startup duration with 10% variation
            cam["temperature"] += self.apply_variation((45 - cam["temperature"]) * 0.01, 4)
            cam["led_pwm"] = self.apply_variation(222,10)  
        else:
            cam["current"] = 0  # No current draw when inactive
            cam["temperature"] -= self.apply_variation((cam["temperature"] - self.environment_temperature) * 0.005, 4)
            cam["led_pwm"] = 0  # LED off

    def update_component_states(self):

        if self.state in [VehicleState.CAMERA_1]:
            self.update_camera(self.camera_states["camera_1"], True)
            self.update_camera(self.camera_states["camera_2"], False)
        elif self.state in [VehicleState.CAMERA_2]:
            self.update_camera(self.camera_states["camera_1"], False)
            self.update_camera(self.camera_states["camera_2"], True)
        else:
            self.update_camera(self.camera_states["camera_1"], False)
            self.update_camera(self.camera_states["camera_2"], False)
        ...
        if self.state == VehicleState.CHARGING:
            self.battery_charge_level = min(100, self.battery_charge_level + 0.5*self.battery_charge_rate)  # Charge battery
        elif self.state in [VehicleState.FORWARD_DRIVE, VehicleState.REVERSE_DRIVE, VehicleState.LEFT_TURN, VehicleState.RIGHT_TURN]:
            self.battery_charge_level = max(0, self.battery_charge_level - 0.5*self.battery_loss_rate)  # Discharge battery
            if self.faults and self.state in [VehicleState.FORWARD_DRIVE, VehicleState.LEFT_TURN]:
                self.battery_charge_level = max(0, self.battery_charge_level - 0.25*self.battery_loss_rate)
        else:
            self.battery_charge_level = max(0, self.battery_charge_level - 0.1*self.battery_loss_rate)  # Slow discharge
    ...
    def publish_telemetry(self):
        ...
        for cam_id, cam in self.camera_states.items():
            channel_values.extend([
                {"channel_name": f"current", "component": f"{cam_id}", "value": double_value(cam["current"])},
                {"channel_name": f"temperature", "component": f"{cam_id}", "value": double_value(cam["temperature"])},
                {"channel_name": f"led_pwm", "component": f"{cam_id}", "value": int32_value(int(cam["led_pwm"]))},
            ])

        channel_values.extend([
            {"channel_name": "battery_charge", "value": double_value(self.battery_charge_level)},
        ])
